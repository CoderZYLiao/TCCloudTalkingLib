...
- (void)layoutSelfSubviews
{
    ...
    //选择器
    self.statePicker.frame = CGRectMake(0, 40, SCREEN_WIDTH, self.pickerHeight - 50);
    
    //控制栏
    self.controllerToolBar.frame = CGRectMake(0, 0, SCREEN_WIDTH, 50);
    
    //完成按钮
    self.finishBtn.frame = CGRectMake(SCREEN_WIDTH - 70, 5, 70, 40);
    
    //取消按钮
    self.cancelBtn.frame = CGRectMake(0, 5, 70, 40);
    
    //标题
    self.titleLabel.frame = CGRectMake(70, 5, SCREEN_WIDTH - 140, 40);
}
...
- (void)setDataSource:(NSArray *)dataSource
{
    ...
    //设置返回默认值
    self.defaultmodel = self.dataSource[0];
...
- (void)setDefaultmodel:(NSString *)defaultmodel
{
    ...
    for (NSInteger i=0; i<_dataSource.count; i++) {
        if ([defaultmodel isEqualToString:self.dataSource[i]]) {
            [self.statePicker selectRow:i inComponent:0 animated:NO];
        }
    }
...
{
    _defaultmodel = defaultStr;
    for (NSInteger i=0; i<_dataSource.count; i++) {

//        NSString *text = [NSString stringWithFormat:@"%@",_dataSource[i]];
        if ([defaultStr isEqualToString:self.dataSource[i]]) {
            [self.statePicker selectRow:i inComponent:0 animated:NO];
        }
    }
}
...
{
    self.defaultmodel = self.dataSource[row];
}
...
{
    self.valueDidSelect(self.defaultmodel);
