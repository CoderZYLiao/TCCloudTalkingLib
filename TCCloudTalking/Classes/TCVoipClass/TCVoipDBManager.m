...
{
    static TCVoipDBManager * manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[TCVoipDBManager alloc] init];
    });
...
// insert into UCS_IM_CONERSATION(targetId,conversationTitle,categoryId,draft,isTop,lastTime,topTime,unReadMsgCount,extra1,extra2,extra3,lastestMessageId,localTime,extra6) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?);
...
- (NSArray *)getAllDataFromDb:(NSString *)tableName
{
    ...
    [self.voipQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([set next]) {
            ...
            if ([obj isKindOfClass:[TCVoipCallListModel class]]) {
                TCVoipCallListModel * model = (TCVoipCallListModel *)obj;
