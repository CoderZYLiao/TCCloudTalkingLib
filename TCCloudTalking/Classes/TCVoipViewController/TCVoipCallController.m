...
@interface TCVoipCallController ()
...
@implementation TCVoipCallController
...
- (void)makeCallView{
    ...
    self.voipNumberLabel.text = self.isGroupDail? @"语音同振中": self.callerName;
    ...
    //显示通话中网络状态 add by kcuky
    
    self.netWorkStatusLabel = [UIButton buttonWithType:UIButtonTypeCustom];
    self.netWorkStatusLabel.enabled = NO;
    self.netWorkStatusLabel.frame = CGRectMake(GetViewX(self.voipNumberLabel), GetViewHeight(self.voipNumberLabel)+GetViewY(self.voipNumberLabel)+ Adaptation(5)+ GetViewHeight(self.voipNumberLabel), GetViewWidth(self.voipNumberLabel), Adaptation(20));
    ...
    [self.callBackView addSubview:self.netWorkStatusLabel];
...
- (TCVoipCallController *)initWithCallerNo:(NSString *)phoneNo andVoipNo:(NSString *)voipNop andCallType:(NSInteger)type;
{
    if (self = [super init])
    {
        self.isGroupDail = NO;
...
- (void)funtionButtonClick:(UIButton *)button{
    switch (button.tag) {
        case 0:{
            ...
            [self muteButtonClick];
            
        }
            break;
        case 1:{
            [self showKeyboardViewButtonClicked];
        }
            break;
        case 2:{
...
/**
 ...
 显示键盘视图
 */
- (void)showKeyboardViewButtonClicked
{
...
-(void)responseVoipManagerStatus:(UCSCallStatus)event callID:(NSString*)callid data:(UCSReason *)data withVideoflag:(int)videoflag
{
    ...
    switch (event)
    {
        ...
        case UCSCallStatus_Answered:
        {
            ...
            if (self.isGroupDail) {
                self.voipNumberLabel.text = @"语音同振通话中";
            }
        ...
        case UCSCallStatus_Failed:
        {
            ...
            }else if(data.reason==402002||data.reason==402019||data.reason==402053){
            
                /**
                 *  @author kucky, 16-08-12 15:08:31
                 *
                 *  1.被叫不在线
                    2.路由不可达
                    3.信令超时
            ...
            [self hangupCall];//当返回一个未知错误码（73-79）时需要将主叫挂断。
            [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(backFront) userInfo:nil repeats:NO];
