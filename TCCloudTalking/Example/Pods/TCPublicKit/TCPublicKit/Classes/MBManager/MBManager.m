...
+(void)showAlertWithCustomImage:(NSString *)imageName title:(NSString *)title inView:(UIView *)view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // 父窗口大小是 80
        CGFloat WH = 40;
        CGFloat XY = (80 - WH)/2.0;
        UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(XY, XY, WH, WH)];
        imgView.image = [UIImage imageNamed:imageName];
        TCCustomerImageView *littleView = [[TCCustomerImageView alloc] init];
        [littleView addSubview:imgView];
        [ZZZWaitingView showWaitViewInView:littleView];
...
+(void)showLoading{
    [self showAlertWithCustomImage:@"loadingLogo" title:nil];
}
